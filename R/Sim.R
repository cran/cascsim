		...
		if(length(object@lines)>0 & length(object@types)>0 & length(object@claimobjs)>0 & nrow(claimData)>0) {
			...
			if(object@iFit==TRUE){
				if(nchar(object@workingFolder)>0 & dir.exists(object@workingFolder)){
					direc <- object@workingFolder
				} else if(nchar(object@workingFolder)>0 ){
					dir.create(object@workingFolder)
					direc <- object@workingFolder
				} else {
					stop("Please input a working folder for this Simulation object")
				}

				if(object@iReport==TRUE){
					if (!dir.exists(paste0(direc,"/fit"))) {dir.create(paste0(direc,"/fit"))}
				}
						...
						if(nrow(fitdata)>5 & co@line==l & co@claimType==t){
							if(fReportLag==TRUE){
								...
								if(TRUE){#(objName(co@reportLag)!="Empirical"){
									...
									for (i in c(1:length(continuousDist))){
										...
										if(so==1) {
											...
											if (check == TRUE){
												jpeg(filename = paste0(direc,"/fit/",object@fitfile,l,t,f,continuousDist[i],".jpg"))
												fitPlot(xFit)
												dev.off()
											}
										} else {
									...
									if (irlso == 1) {
										...
										xFit@fitted<-bestfit
										png(filename = paste0(direc,"/fit/","fit",object@tag,l,t,f,".png"))
										fitPlot(xFit)
										dev.off()
									} else {
								...
								if (check == TRUE) {
									...
									xFit <- setFitdata(xFit)
									setTrialDist(xFit) <- new("Empirical")
									jpeg(filename = paste0(direc,"/fit/",object@fitfile,l,t,f,"Empirical.jpg"))
									fitPlot(xFit)
									dev.off()
									cn <- paste0(l,t,f,"emp")
								...
								if(TRUE){#(objName(co@frequency)!="Empirical"){
									...
									for (i in c(1:length(discreteDist))){
										...
										if(so==1) {
											...
											fitsummary[rfit,"p0"] <- NA
											if (check == TRUE){
												jpeg(filename = paste0(direc,"/fit/",object@fitfile,l,t,f,discreteDist[i],".jpg"))
												fitPlot(xFit)
												dev.off()
											}
										} else {
							...
							if(fDeductible==TRUE){
								...
								rfit <- rfit + 1
								png(filename = paste0(direc,"/fit/","fit",object@tag,l,t,f,".png"),width=240,height=240)
								CDFPlot(xFit)
								dev.off()
								...
								if(nrow(x)>10) {
									...
									for (i in c(1:length(copulaList))){
										...
										if(so==1) {
											...
											if (check == TRUE){
												jpeg(filename = paste0(direc,"/fit/",object@fitfile,l,t,f,copulaList[i],".jpg"),width=480,height=240)
											...
											if (check == TRUE){
												jpeg(filename = paste0(direc,"/fit/",object@fitfile,l,t,f,copulaList[i],".jpg"),width=480,height=480)
												par(mfrow = c(1, 1))
									...
									if (issrso == 1) {
										...
										png(filename = paste0(direc,"/fit/","fit",object@tag,l,t,f,".png"),width=480,height=240)
				...
				if(fFreqCorrelation==TRUE & object@iCopula==TRUE){
					...
					if (nrow(x)>=10) {
						...
						if (ifcso == 1) {
							...
							png(filename = paste0(direc,"/fit/","fit",object@tag,f,".png"),width=480,height=240)
							copulaFitPlot(bestfit)
							dev.off()
						} else {
				...
				if (check == TRUE) {
					jpeg(filename = paste0(direc,"/fit/",object@fitfile,"na.jpg"),width=480,height=480)
					par(mfrow = c(1, 1))
					plotText(paste0("Not Fitted"))
					dev.off()
					jpeg(filename = paste0(direc,"/fit/",object@fitfile,"halfna.jpg"),width=480,height=240)
					par(mfrow = c(1, 1))
					plotText(paste0("Not Fitted"))
					dev.off()
				}

				if(nrow(fitsummary)>=1) {write.table(fitsummary,paste0(direc,"/",fitname),row.names=FALSE, sep=",")}
				if(ncol(fitsumfac)>=1) {write.table(fitsumfac,paste0(direc,"/",facname),row.names=FALSE, sep=",")}
				if(nrow(fitsumcop)>=1) {write.table(fitsumcop,paste0(direc,"/",copname),row.names=FALSE, sep=",")}
				if(ncol(empiricals)>=1) {write.table(empiricals,paste0(direc,"/",empname),row.names=FALSE, sep=",")}
				...
				if(object@iReport==TRUE){
					...
					pckgdir <- paste0(pckgdir,"/css/")
					if (!dir.exists(paste0(direc,"/fit"))) {dir.create(paste0(direc,"/fit"))}
					copied <- file.copy(file.path(pckgdir, "report.css"), file.path(paste0(direc,"/fit/"),"report.css"))
					if(object@fitRpt==""){
						fitrpt <- paste0("sim",object@tag)
					} else {
						fitrpt <- paste0(object@fitRpt)
					}
					if (file.exists(paste0(direc,"/fit/",fitrpt,".html"))) {file.remove(paste0(direc,"/fit/",fitrpt,".html"))}

					ReportBegin <- function(file, title, directory = direc) {
						file = file.path(paste0(directory,"/fit"), file)
						cat(paste("<html><head><title>", title, "</title></head>", "<body bgcolor=#D0D0D0>", sep = ""), file = file, append = FALSE)
					}

					ReportEnd <- function(file, directory = direc) {
						file = file.path(paste0(directory,"/fit"), file)
					...
					HTMLcontent <- function(file, append = TRUE, directory = direc) {
						file = file.path(paste0(directory,"/fit"), file)
						...
						if(fFreqCorrelation==TRUE & object@iCopula==TRUE){
							...
							if (ifcso == 1) {
								...
								HTMLInsertGraph(paste0(direc,"/fit/","fit",object@tag,"freqCorrelation.png"),Caption = "Copula Fitting Plot",file = file)
							} else {
								...
								if (!is.null(findClaimObj(l,t,object@claimobjs))){
									if(fFrequency==TRUE){
										...
										if (findClaimObj(l,t,object@claimobjs)@frequency@fitsucc == TRUE) {
											...
											HTMLInsertGraph(paste0(direc,"/fit/","fit",object@tag,l,t,"frequency.png"),Caption = "",file = file)
									...
									if(fSeverity==TRUE){
										...
										if (findClaimObj(l,t,object@claimobjs)@severity@fitsucc == TRUE) {
											...
											HTMLInsertGraph(paste0(direc,"/fit/","fit",object@tag,l,t,"severity.png"),Caption = "",file = file)
											HTML("Tested Distributions", file = file)
									...
									if(fReportLag==TRUE){
										...
										if (findClaimObj(l,t,object@claimobjs)@reportLag@fitsucc == TRUE) {
											...
											HTMLInsertGraph(paste0(direc,"/fit/","fit",object@tag,l,t,"reportLag.png"),Caption = "",file = file)
											HTML("Tested Distributions", file = file)
									...
									if(fSettlementLag==TRUE){
										...
										if (findClaimObj(l,t,object@claimobjs)@settlementLag@fitsucc == TRUE) {
											...
											HTMLInsertGraph(paste0(direc,"/fit/","fit",object@tag,l,t,"settlementLag.png"),Caption = "",file = file)
											HTML("Tested Distributions", file = file)
									...
									if(fDeductible==TRUE){
										...
										HTMLInsertGraph(paste0(direc,"/fit/","fit",object@tag,l,t,"Deductible.png"),Caption = "",file = file,WidthHTML=250)
										HTMLhr(file = file)
									}

									if(fLimit==TRUE){
										HTML.title(paste0("Limit Empirical Distribution"),file = file, HR=3)
										HTMLInsertGraph(paste0(direc,"/fit/","fit",object@tag,l,t,"Limit.png"),Caption = "",file = file,WidthHTML=250)
										HTMLhr(file = file)
									}
									...
									if(findClaimObj(l,t,object@claimobjs)@iCopula==TRUE & fReportLag==TRUE & fSettlementLag==TRUE & fSeverity==TRUE & fSSRCorrelation==TRUE){
										...
										if (findClaimObj(l,t,object@claimobjs)@ssrCopula@fitsucc == TRUE) {
											...
											HTMLInsertGraph(paste0(direc,"/fit/","fit",object@tag,l,t,"ssrCorrelation.png"),Caption = "",file = file)
											HTML("Tested Copulas", file = file)
					...
					Report <- function(file) {
						...
						if(copied == TRUE | file.exists(paste0(direc,"/fit/","report.css"))) {
							HTMLCSS(file = paste0(direc,"/fit/",file), CSSfile = paste0(direc,"/fit/","report.css"))
						}
						HTMLcontent(file)
						ReportEnd(file)
					}
					
					big_mark_opt <- getOption("R2HTML.format.big.mark")
					decimal_mark_opt <- getOption("R2HTML.format.decimal.mark")
					options("R2HTML.format.big.mark"=",")
					options("R2HTML.format.decimal.mark"=".")
					Report(paste0(fitrpt,".html"))
			        on.exit(options("R2HTML.format.big.mark"=big_mark_opt))
					on.exit(options("R2HTML.format.decimal.mark"=decimal_mark_opt), add = TRUE)
				}
	...
	tryCatch({
		...
		if(length(object@lines)>0 & length(object@types)>0 & length(object@claimobjs)>0 & iclass) {
			...
			if(object@ncores>1 & object@simNo >1){
						...
						for(l in object@lines){
							for(t in object@types){
								...
								if(co@line==l & co@claimType==t){
									...
									if(file.exists(paste0(direc,"/",filenamei))){
										write.table(stmp,paste0(direc,"/",filenamei),row.names=FALSE, sep=",",append = TRUE, col.names = FALSE)
									}else{
										write.table(stmp,paste0(direc,"/",filenamei),row.names=FALSE, sep=",",append = TRUE, col.names = TRUE)
									}
				...
				rm(msimdata)
				gc()
				if(file.exists(paste0(direc,"/",filename))){
					write.table(simdata,paste0(direc,"/",filename),row.names=FALSE, sep=",",append = TRUE, col.names = FALSE)
				}else{
					write.table(simdata,paste0(direc,"/",filename),row.names=FALSE, sep=",",append = TRUE, col.names = TRUE)
				}
					...
					for(l in object@lines){
						for(t in object@types){
							...
							if(co@line==l & co@claimType==t){
								...
								stmp<-rbind(stmp,claimSample(co,claimData,startDate,evaluationDate)) #futureDate
								if(file.exists(paste0(direc,"/",filename))){
									write.table(stmp,paste0(direc,"/",filename),row.names=FALSE, sep=",",append = TRUE, col.names = FALSE)
								}else{
									write.table(stmp,paste0(direc,"/",filename),row.names=FALSE, sep=",",append = TRUE, col.names = TRUE)
								}
								simdata <- rbind(simdata,stmp)
...
#			}
	...
	#								}
			...
			write.table(simSummary,paste0(direc,"/",summaryname),row.names=FALSE, sep=",")
			return(simSummary)
		} else {
...
setMethod("simReport", signature("Simulation", "data.frame"), function(object, simSummary, simTriangle=NA, startDate=as.Date("2012-01-01"),evaluationDate=as.Date("2016-12-31"),futureDate=as.Date("2017-12-31"),iYear=FALSE)
{
	tryCatch({
		...
		if (object@iReport==TRUE){
			...
			pckgdir <- find.package("cascsim")
			pckgdir <- paste0(pckgdir,"/css/")
			if (!dir.exists(paste0(direc,"/report"))) {dir.create(paste0(direc,"/report"))}
			copied <- file.copy(file.path(pckgdir, "report.css"), file.path(paste0(direc,"/report/"),"report.css"))
			if(object@htmlRpt==""){
				htmlfile <- paste0("sim",object@tag)
			} else {
				htmlfile <- paste0(object@htmlRpt)
			}
			if (file.exists(paste0(direc,"/report/",htmlfile,".html"))) {file.remove(paste0(direc,"/report/",htmlfile,".html"))}

			ReportBegin <- function(file, title, directory = direc) {
				file = file.path(paste0(directory,"/report"), file)
				cat(paste("<html><head><title>", title, "</title></head>", "<body bgcolor=#D0D0D0>", sep = ""), file = file, append = FALSE)
			}
			...
			HTMLcontent <- function(file, append = TRUE, directory = direc) {
				...
				png(filename = paste0(direc,"/report/","sim",object@tag,"tp.png"))
				RptPlot("Total","Total","Total","Total")
				dev.off()
				HTMLInsertGraph(paste0(direc,"/report/","sim",object@tag,"tp.png"),Caption = "CDF",file = file)
				HTMLhr(file = file)
				if (iYear==TRUE) {
				...
				for(l in object@lines){
					...
					png(filename = paste0(direc,"/report/","sim",object@tag,l,".png"))
					RptPlot(l,"Total","Total","Total")
					dev.off()
					HTMLInsertGraph(paste0(direc,"/report/","sim",object@tag,l,".png"),Caption = "CDF",file = file)
					HTMLhr(file = file)
					if (iYear==TRUE) {
					...
					for(t in object@types){
						if(!is.null(findClaimObj(l,t,object@claimobjs))){
							...
							png(filename = paste0(direc,"/report/","sim",object@tag,l,t,".png"))
							RptPlot(l,t,"Total","Total")
							dev.off()
							HTMLInsertGraph(paste0(direc,"/report/","sim",object@tag,l,t,".png"),Caption = "CDF",file = file)
							HTMLhr(file = file)
							if (iYear==TRUE) {
							...
							for(c in classes){
								...
								if (haveData(l,t,c2) == TRUE) {
									...
									png(filename = paste0(direc,"/report/","sim",object@tag,l,t,c,".png"))
									RptPlot(l,t,c2,"Total")
									dev.off()
									HTMLInsertGraph(paste0(direc,"/report/","sim",object@tag,l,t,c,".png"),Caption = "CDF",file = file)
									HTMLhr(file = file)
									if (iYear==TRUE) {
									...
									if(c=="IBNER"){
										if(!is.null(findClaimObj(l,t,object@claimobjs))){
											...
											if (findClaimObj(l,t,object@claimobjs)@ioDevFac==3 & findClaimObj(l,t,object@claimobjs)@fIBNER@distType == "normal"){
												png(filename = paste0(direc,"/report/","sim",object@tag,l,t,c,"fibner.png"),width=240,height=240)
												fibnerPlot(findClaimObj(l,t,object@claimobjs))
												dev.off()
												HTMLInsertGraph(paste0(direc,"/report/","sim",object@tag,l,t,c,"fibner.png"),Caption = "",file = file,WidthHTML=250)
											}
										}
									}else if (c=="REOPEN"){
										if(!is.null(findClaimObj(l,t,object@claimobjs))){
											...
											png(filename = paste0(direc,"/report/","sim",object@tag,l,t,c,"ro.png"))
											roPlot(findClaimObj(l,t,object@claimobjs))
											dev.off()
											HTMLInsertGraph(paste0(direc,"/report/","sim",object@tag,l,t,c,"ro.png"),Caption = "",file = file)
										}
									}else if (c=="IBNR"){
										if(!is.null(findClaimObj(l,t,object@claimobjs))){
											...
											png(filename = paste0(direc,"/report/","sim",object@tag,l,t,c,"dist.png"))
											distPlot(findClaimObj(l,t,object@claimobjs))
											dev.off()
											HTMLInsertGraph(paste0(direc,"/report/","sim",object@tag,l,t,c,"dist.png"),Caption = "",file = file)
											...
											png(filename = paste0(direc,"/report/","sim",object@tag,l,t,c,"idx.png"))
											idxPlot(findClaimObj(l,t,object@claimobjs))
											dev.off()
											HTMLInsertGraph(paste0(direc,"/report/","sim",object@tag,l,t,c,"idx.png"),Caption = "",file = file)
											...
											png(filename = paste0(direc,"/report/","sim",object@tag,l,t,c,"dl.png"),width=480,height=240)
											dlPlot(findClaimObj(l,t,object@claimobjs))
											dev.off()
											HTMLInsertGraph(paste0(direc,"/report/","sim",object@tag,l,t,c,"dl.png"),Caption = "",file = file)
										}
			...
			Report <- function(file) {
				...
				if(copied == TRUE | file.exists(paste0(direc,"/report/","report.css"))) {
					HTMLCSS(file = paste0(direc,"/report/",file), CSSfile = paste0(direc,"/report/","report.css"))
				}
				HTMLcontent(file)
				ReportEnd(file)
			}
